...
module sd_if_controller_cmd
  (output [19:0] status,
   ...
   localparam state_cmd_received = 1; // new command has been received
   localparam state_expecting_cmd_write = 2; // more data to write is expected
   localparam state_writing = 3; // more data to write is needed in the future
                   ...
                   if (more_data_towrite)
                     begin
                        ...
                        if (write_no_cmd_index)
                          state_next = state_expecting_cmd_write;
                     end
                   else
                     state_next = state_idle;
