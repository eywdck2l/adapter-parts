   ...
   always_comb begin
      ...
      case (state)
        ...
        state_wait_resp: begin
           // A 0 bit written by us is guaranteed to be read as 1 by the
           // module sd_io_raw.
...
module sd_io_raw
  (// When a bit is read and it was not written by this unit, in_d holds
   // that bit.  If the bit is written by this unit, in_d is 1.
   output reg in_d,
   input out_d,
   input out_en,
   ...
   always @(posedge clk_sys)
     if (~resetn) begin
        ...
        line_out_t_d <= {line_out_t_d[0], out_t_r};
        clk_bus_d <= clk_bus[0];
   ...
   always @(posedge clk_sys)
     if (~resetn)
       ...
       // Read the input on the falling edge of the bus clock.  Mask it
       // if it was written by us.
       in_d <= line_i_d[1] | ~line_out_t_d[1];
