module axi_xts_aes
  #(parameter C_S_AXI_DATA_WIDTH = 32,
   ...
   // The 128-bit block to start processing from.  It is in little
   // endian on the AXI interface and converted to big endian for
   // encryption.
   ...
   reg         load_tweak;
   reg [TWEAK_WIDTH+$clog2(DATA_UNIT_SIZE)-1:0] block_pos_in;

   always_comb
     foreach (block_pos_in[i])
       block_pos_in[i] = reg_written[REG_BLOCK_POS+i/8][i%8];
   ...
   xts_aes #(.KEY_SIZE(KEY_SIZE),
     ...
     (.out_tdata,
      ...
      .key_in_wren,
      .tweak_in(block_pos_in[$clog2(DATA_UNIT_SIZE)+:TWEAK_WIDTH]),
      .skip_blocks_in(block_pos_in[0+:$clog2(DATA_UNIT_SIZE)]),
      .load_tweak,
