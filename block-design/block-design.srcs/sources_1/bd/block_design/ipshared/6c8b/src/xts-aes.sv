module xts_aes
  #(// The key size is twice the key size of the corresponding AES
    ...
    // Number of 128-bit blocks in a data unit
   ...
   (// Data out
    ...
    input [$clog2(DATA_UNIT_SIZE)-1:0] skip_blocks_in = '0,
    input load_tweak,
   ...
   // Control skipping data units

   wire t_loaded;

   reg [$clog2(DATA_UNIT_SIZE)-1:0] xor_skipped;
   reg                              xor_skipping_done;
   reg                              skip_1_block;

   always @(posedge clk)
     if (~resetn | ~t_loaded | ~t_enc_valid) begin
        xor_skipped <= 0;
        skip_1_block <= 1'b0;
        xor_skipping_done <= 1'b0;
     end
     else if (!xor_skipping_done && xor_skipped < skip_blocks_in) begin
        xor_skipped <= xor_skipped + 1;
        skip_1_block <= 1'b1;
        xor_skipping_done <= 1'b0;
     end
     else begin
        skip_1_block <= 1'b0;
        xor_skipping_done <= 1'b1;
     end

   wire may_take_data;

   assign may_take_data = ready_in & xor_skipping_done;

   // Keep count of when a data unit ends

   wire   in_end_of_block;
   ...
   always @(posedge clk)
     if (~resetn | clear_valid)
       block_count <= '0;
     else if ((in_tvalid & in_tready) | skip_1_block)
       block_count <= block_count_inc;

   assign in_end_of_block = block_count_inc[$clog2(DATA_UNIT_SIZE)];
      ...
      if (WORKER_COUNT < ROUND_COUNT) begin : input_buffered
         data_buffer #(.WIDTH(129)) buffer_input
           (.out_data({pipe_last[0], pipe_data[0]}),
            ...
            .in_data({in_end_of_block, in_tdata}),
            .in_valid(in_tvalid),
            .in_ready(in_tready),
            .clk,
         ...
         assign pipe_last[0] = in_end_of_block;
...
module xor_t
  (output [127:0] out_data,
   ...
   // Advance t without processing data
   input skip,
   ...
   always @(posedge clk)
     if ((in_valid & in_last) | ~t_loaded)
       t <= t_in;
     else if ((in_valid & in_ready) | skip)
       t <= (t << 1) ^ (128'h87 * t[127]);
