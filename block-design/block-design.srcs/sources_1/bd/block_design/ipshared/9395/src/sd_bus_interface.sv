...
package dat_error_codes;
   ...
   // by CMD12 is not included.  It's a separate bit in the status
   // register.
...
module dat_interface
  #(// The word size in bits
    ...
    // Same for the busy signal after the card reads a block.  Some card
    // readers have trouble when it's too short.
    parameter busy_time_min = 64)
   (// Data out
    input fifo_out_almostfull,
   ...
   // Count the number of sd bus clocks that have past in the current
   // state.
   reg [counter_width-1:0] counter;
   wire [counter_width:0]  counter_inc = counter + 1;
   reg                     counter_reset;
   // Save whether the counter has passed some number.  Used when the
   // counter may overflow and comparision doesn't work.
   reg                     counter_passed[1<<counter_width];
   ...
   always @(posedge clk)
     if (~resetn | counter_reset) begin
        counter <= 0;
        foreach (counter_passed[i])
          counter_passed[i] <= 1'b0;
     end
     else if (new_sd_clk) begin
        counter <= counter_inc;
        foreach (counter_passed[i])
          counter_passed[i] <= counter_passed[i] || (counter & i) == i;
     end
   ...
   always @(posedge clk)
     if (~resetn)
       ...
       block_done <= (state == state_write_data || state == state_read_data) &&
   ...
   generate
      for (genvar i = 0; i < 4; i = i + 1) begin: line_crc
         line_crc #(.width(crc_width),
   ...
   always @(posedge clk)
     if (~resetn)
       ...
       // End bits not being all 1's is treated as CRC error.
       read_crc_err <= crc_error_next || sd_dat_i_sys != '1;
   ...
   always @(*) begin
      ...
      case (state)
        ...
        state_write_delay:
          if (new_sd_clk && in_tvalid && counter_passed[write_delay_min])
        ...
        state_read_busy: begin
           if (new_sd_clk && counter_passed[busy_time_min] &&
               !fifo_out_almostfull) begin
